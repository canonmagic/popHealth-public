...
class BulkRecordImporter
  ...
  def self.update_mrn(patient_data, doc)
    patient_role_element = doc.at_xpath('/cda:ClinicalDocument/cda:recordTarget/cda:patientRole')
    identifiers = patient_role_element.xpath("./cda:id")
    patient_data[:medical_record_number] = identifiers[0].attribute("extension")
    patient_data
  end
  ...
  def self.checkdedup(patient_data, practice_id=nil)
    ...
    if result.first
      existing_patient = Patient.where("_id": result.first["_id"]).first
      begin
