  ...
  class ExpectedResultsCalculator
    ...
    def aggregate_results_for_measure(measure, individual_results = nil)
      ...
      @measure_result_hash[measure.hqmf_id].keys.each do |key|
        ...
        if @callingfor
         begin
          qc = QualityReport.where('measure_id' => measure.id, 'effective_date' => @effective_date,'start_date' => @start_date, "filters" => @filters, "status.state" => {'$in': ["pending", "completed"]}).first
          if !qc
