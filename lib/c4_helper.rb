...
=end
  ...
  class Cat1ZipFilter
    ...
    def initialize(measures_in, start_date_in, end_date_in)
      ...
      @options = { start_time: Time.at(start_date_in.to_i).to_datetime, end_time: Time.at(end_date_in.to_i).to_datetime}
        ...
        if patients && patients.length > 0
          Zip::OutputStream.open(outfilepath) do |zout|
              patients.each do |patient_hash|
                ...
                sf_patient.add_encounter_ids_to_events unless (measures.map(&:hqmf_set_id) & APP_CONFIG['result_measures'].map { |mes| mes["hqmf_set_id"] }).empty?
                ...
                # LET'S ADD NECCESARY DEMOGRAPHIC INFO HERE OTHERWISE LIBRARY PUT'S DEFAULT INFO
                @options[:patient_addresses] = sf_patient.addresses
                # BEFORE USING TELECOM, NEED TO CLEAR PREFIXES
                @options[:patient_telecoms] = []
                patient_emails = []

                unless sf_patient.telecoms.empty?
                  sf_patient.telecoms.each do |telecom|
                    if telecom.value.match(/^mailto:/)
                      telecom.value = telecom.value.gsub(/mailto:/, "")
                      patient_emails << telecom
                    elsif telecom.value.match(/^tel:/)
                      telecom.value = telecom.value.gsub(/tel:/, "")
                      @options[:patient_telecoms] << telecom
                    else
                      @options[:patient_telecoms] << telecom
                    end
                  end
                end
                @options[:patient_email] = patient_emails.first.value unless patient_emails.empty?
