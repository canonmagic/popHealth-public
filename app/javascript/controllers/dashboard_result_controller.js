...
export default class extends Controller {
    ...
    extractDataFromURL(){
        ...
        this.population = params.get('population_set');
    ...
    */
    ...
    buildPopulationFilter(filterPopulation) {

        let filterPopulationSelect = filterPopulation.length === 0 ? [`${this.population}`] : filterPopulation;

        const filterPopulationList = document.querySelector('.ul-population');
        ...
        filterPopulationSelect.forEach((filter) => {
            ...
            const buttonId = `btn-${filter}`;

            button.id = buttonId;

            const filterName = document.createTextNode(filter);
    ...
    async changePopulation(event) {
        ...
        localStorage.setItem('populationSelected', JSON.stringify(buttonId));

        let oldUrl = new URL(window.location.href);

        oldUrl.searchParams.set('population_set', buttonId);

        window.location.href = oldUrl.toString();

    };

    verifyPopulation() {

        let populationSelected = this.population;
        
        let buttonPopulation = document.getElementById(`btn-${populationSelected}`);

        if ((populationSelected && populationSelected !== null && populationSelected !== '' && populationSelected !== undefined) && (buttonPopulation && buttonPopulation !== null)) {

            //console.log(buttonPopulation)

            buttonPopulation.classList.add('active');

            buttonPopulation.disabled = true;
